...
class EllipseBuffered(ShapeBuffered):
    ...
    def __init__(self, x, y, width, height, angle, delta_x, delta_y,
        ...
        shape = shape_list.create_ellipse_filled(0, 0,
                                             self.width, self.height,
                                             self.color, self.angle)
        self.shape_list = shape_list.ShapeElementList()
...
class RectangleBuffered(ShapeBuffered):
    ...
    def __init__(self, x, y, width, height, angle, delta_x, delta_y,
        ...
        shape = shape_list.create_rectangle_filled(0, 0,
                                               self.width, self.height,
                                               self.color, self.angle)
        self.shape_list = shape_list.ShapeElementList()
        self.shape_list.append(shape)
...
class LineBuffered(ShapeBuffered):
    ...
    def __init__(self, x, y, width, height, angle, delta_x, delta_y,
        ...
        shape = shape_list.create_line(0, 0,
                                   self.width, self.height,
                                   self.color, 2)
        self.shape_list = shape_list.ShapeElementList()
        self.shape_list.append(shape)
...
class Test(ArcadePerfTest):
    ...
    def __init__(self, mode: str = "buffered"):
        ...
        self.name = self.get_instance_name(mode=mode)
